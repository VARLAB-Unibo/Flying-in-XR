...
public class MainBehavior : MonoBehaviour {
	...
	void Start() {
		...
		MainBehavior.instance = this;
        SingleTonParams singletonParams = new SingleTonParams();
        ...
        addcmd("wsinput", "Get images from a url", "SocketOnlineTexStart", this);
		addcmd("custominput", "Get images from a url", "CustomOnlineTexStart", this);
	...
	public TexInputs get_texInputs()
	{
		return _texInputs;
    ...
    void Update() {
		...
		if (_donnx == null)
			...
			if (_currentFileType != FileTypes.Online)
			{
				_texInputs.UpdateTex();
			}
			else
	...
	// url is a websocket
    public void SocketOnlineTexStart(string url) =>
        OnlineTexStart(new SocketOnlineTex(url));


    public void CustomOnlineTexStart(string url) =>
    OnlineTexStart(new EmptyOnlineTex(url));
    ...
    private void OnlineTexStart(OnlineTex otex) {
		...
		GameObject.Find("UnityManager").AddComponent<SocketOnlineTexBehaviour>();
