...
// UIStaticClassSetter per ottenere shader predefiniti sulla mesh


enum DepthMeshParameters
{
    ProjRatio_t = 1,
    CamDistL_t = 2,
    ScaleR_t = 3,
...
public class MeshShaders {
	public string Name {get; private set;}	// shader name
...
public interface IDepthMesh {
	bool ShouldUpdateDepth {set;} //whether the depth has to be updated when the parameters (alpha, ...) is changed
                                  //  Proprietà per specificare se la profondità deve essere aggiornata quando i parametri cambiano.

    void SetScene(Depth depth, Texture texture=null, float ratio=0);
    // Metodo per impostare la scena con una mappa di profondità e opzionalmente una texture e un rapporto

    // Metodo per impostare la scena con una mappa di profondità e opzionalmente una texture e un rapporto
    void SetTexture(Texture texture = null);     
...
// Gestisce il comportamento della mesh in Unity
public class MeshBehavior : MonoBehaviour, IDepthMesh {
	...
	*/
	...
	//transform.localPosition = new Vector3(_defaultX + _width * _localScaleZ, _defaultY - _height * _localScaleZ, _defaultZ + _camDist);
	//transform.localPosition = new Vector3(_defaultX + _localScaleZ, _defaultY, _defaultZ + _camDist);

	private static float _scalePerCamDist = (0.96f / 150); //Scale (legacy) (shrinked a little (96%)) was 1 when CamDist (MeshLoc=0) was 150

	public static float DefaultCamDist = 10f;   // this is 'value' reference
	...
	public float CamDistL { set {
			CamDist = MathF.Pow(10, value); 
		} 
		
		get { return MathF.Log10(CamDist); } } //Log10 version

	public static float DefaultScaleR = 1f; //Relative
	...
	public static float DefaultDepthMultR = 1f;
	...
	public static float DefaultMeshHor = 0f;
	...
	public static float DefaultMeshVer = 0f;
	...
	public static float DefaultProjRatio = 0.2f;
	...
	public void ToDefault() {
		//Alpha = DefaultAlpha;
		//Beta = DefaultBeta;
		ProjRatio = SingleTonParams.instance.Params["ProjRatio"];
        CamDistL = SingleTonParams.instance.Params["CamDistL"];

        ScaleR = SingleTonParams.instance.Params["ScaleR"];

        DepthMultRL = SingleTonParams.instance.Params["DepthMultRL"];
	...
	//public void SetValueParameter(string paramName, float value)
 //   {
 //       DepthMeshParameters inputAsEnum = (DepthMeshParameters)Enum.Parse(typeof(DepthMeshParameters), paramName);

	//	switch (inputAsEnum)
	//	{

	//		case DepthMeshParameters.CamDistL_t:
	//			DefaultCamDist = value;
	//			break;
 //           case DepthMeshParameters.DepthMultRL_t:
 //               DefaultDepthMultR = value;
 //               break;
 //           case DepthMeshParameters.ProjRatio_t:
 //               DefaultProjRatio = value;
 //               break;
 //           case DepthMeshParameters.ScaleR_t:
 //               DefaultScaleR = value;
 //               break;
 //       }
	//	//this.ToDefault();
 //   }
	...
	public void SaveAsAsset() {
		...
		bool success;
		UnityEditor.PrefabUtility.SaveAsPrefabAsset(gameObject, path, out success);
